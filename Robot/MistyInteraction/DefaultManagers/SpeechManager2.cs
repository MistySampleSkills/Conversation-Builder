...
{
	public class SpeechManager2: BaseManager, ISpeechManager
	{
		...
		public override async Task<bool> Initialize()
		{
			...
			if (_azureSpeechParameters != null && !string.IsNullOrWhiteSpace(_azureSpeechParameters.SubscriptionKey))
			{
				...
				_speechIntentManager = _speechIntentManager ?? new SpeechIntentManager2(Robot, _intentUtterances);
			}
		...
		public SpeechManager2(IRobotMessenger misty, IDictionary<string, object> parameters, CharacterParameters characterParameters, CharacterState characterState, CharacterState stateAtAnimationStart, CharacterState previousState, IList<GenericDataStore> genericDataStores, ISpeechIntentManager2 speechIntentManager = null)
		...
		private async void VoiceRecordCallback(IVoiceRecordEvent voiceRecordEvent)
		{
			try
			{
				...
				string service = CharacterParameters.SpeechRecognitionService.ToLower().Trim();
