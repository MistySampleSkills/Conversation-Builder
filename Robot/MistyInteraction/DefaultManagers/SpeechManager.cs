			...
			try
			{
				...
				lock (_lockListenerData)
				{
					if (!_recording && !_listenAborted && (_listeningCallbacks.Remove(audioComplete.Name) || _listeningCallbacks.Remove(audioComplete.Name + ".wav")))
					{
						...
						{
							case "googleonboard":
								_ = Robot.CaptureSpeechGoogleAsync(false, _listenTimeout, _silenceTimeout, CharacterParameters.GoogleSpeechRecognitionParameters.SubscriptionKey, CharacterParameters.GoogleSpeechRecognitionParameters.SpokenLanguage);
								return;
							case "azureonboard":
								_ = Robot.CaptureSpeechAzureAsync(false, _listenTimeout, _silenceTimeout, CharacterParameters.AzureSpeechRecognitionParameters.SubscriptionKey, CharacterParameters.AzureSpeechRecognitionParameters.Region, CharacterParameters.AzureSpeechRecognitionParameters.SpokenLanguage);
								return;
							case "vosk":
