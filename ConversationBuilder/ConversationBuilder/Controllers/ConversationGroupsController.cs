	...
	public class ConversationGroupsController : AdminToolController
	{
		...
		public async Task<ActionResult> StartSkill(ConversationGroup model)
		{
			try
			{
				...
				{				
					SkillParameters skillParameters = await GenerateSkillConfiguration(model.Id, model.AnimationCreationMode, model.AnimationCreationDebounceSeconds, model.IgnoreArmCommands, model.IgnoreHeadCommands, model.SmoothRecording, model.PuppetingList, model.RetranslateTTS, model.CreateJavaScriptTemplate);					
